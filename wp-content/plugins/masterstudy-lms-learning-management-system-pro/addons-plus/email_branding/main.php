...
function stm_lms_email_manager_settings_plus( $data ) {
		...
		'email_template' => array(
			...
			'fields' => array(
				...
				'controll_id' => array(
					...
					'description'      => esc_html__( 'By default, emails will be sent as displayed in Preview. By uploading a custom logo, and background images for the header & footer you can overwrite it.', 'masterstudy-lms-learning-management-system-pro' ),
				...
				'stm_lms_email_template_header_email' => array(
					...
					'value'        => ms_lms_get_default_email(),
					'dependency'   => array(
						array(
							'key'   => 'stm_lms_email_template_branding',
							'value' => 'not_empty',
	...
	return array_merge( $emailBranding, $data );
}

add_filter( 'stm_lms_filter_email_data', 'stm_lms_email_filter_email_data', 100, 1 );
function stm_lms_email_filter_email_data( $data ) {

	if ( ! empty( get_option( 'stm_lms_email_manager_settings', array() )['stm_lms_email_template_branding'] ) ) {
		$data['message'] = STM_LMS_Templates::load_lms_template(
			'emails/main',
			array(
				'message'       => $data['message'],
				'subject'       => $data['subject'],
				'email_manager' => ( class_exists( 'STM_LMS_Email_Manager' ) ) ? STM_LMS_Email_Manager::stm_lms_get_settings() : array(),
			)
		);
	}
...
add_filter( 'wp_mail_from', 'ms_lms_get_from_address' );
add_filter( 'wp_mail_from_name', 'ms_lms_get_from_name' );
 ...
 * @return string
 */
function ms_lms_get_from_address() {
	$emailManagerSettings = ( class_exists( 'STM_LMS_Email_Manager' ) ) ? STM_LMS_Email_Manager::stm_lms_get_settings() : array();
	$from_email            = $emailManagerSettings['stm_lms_email_template_header_email'];

	return ! $from_email ? ms_lms_get_default_email() : $from_email;
}
 ...
 * @return string
 */
function ms_lms_get_from_name() {
	$emailManagerSettings = ( class_exists( 'STM_LMS_Email_Manager' ) ) ? STM_LMS_Email_Manager::stm_lms_get_settings() : array();
	$from_name            = $emailManagerSettings['stm_lms_email_template_header_name'];

	return ! $from_name ? get_option( 'blogname' ) : $from_name;
}

/**
 * Get default WP e-mail
 *
 * @return string
 */
function ms_lms_get_default_email() {
